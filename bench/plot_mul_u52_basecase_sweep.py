...
def main():
    ...
    for ax, shape in zip(axes, shapes):
        ...
        x = d["an52"]
        ax.plot(x, d["u52_ns_per_digit_product"], label="mul_u52_basecase", linewidth=1.8)
        ax.plot(x, d["ref_core_ns_per_digit_product"], label="reference core", linewidth=1.6)
        ax.plot(x, d["ref_full_ns_per_digit_product"], label="reference full", linewidth=1.2, alpha=0.75)
        ax.set_title(shape)
