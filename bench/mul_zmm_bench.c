// Benchmark mul_zmm (8x8 -> 16-limb block multiply, div.h), correctness vs mul_u52_1.
#define _GNU_SOURCE
...
int main(int c,char**v){
  ...
  // reference: mul_u52_1(8,8) alone (redundant) for context
  { static _vec rr[2]; double br=1e300;
    for(int rep=0;rep<9;rep++){uint64_t t0=now();for(int l=0;l<LOOPS;l++)for(int i=0;i<M;i++){mul_u52_1(rr,(cpvec)Atab[i],(cpvec)Btab[i],8,8);sink^=((uint64_t*)rr)[0];}uint64_t t1=now();double ns=(double)(t1-t0)/((double)LOOPS*M);if(ns<br)br=ns;}
    printf("mul_u52_1(8,8) thr: %.2f ns  %.1f cyc  (redundant, for context)\n",br,br*g);
  }
