#pragma once
// p30_ref.h - REFERENCE SUPERSET of the p30 engine: the production
// implementation plus every validated-but-dormant experimental path
// (the general truncated inverse p30_inv_rec/p30_inv_full, the m3
// 24-row and x6 6-row mixed fused-input passes and their pre-depth
// drivers). Kept battery-green; internals benches build against this
// file. The CLEAN production engine is p30.h -- new work goes there.
...
// Design: docs/p30_ntt_design.md; transform math verified by the scalar
// model tests/p30_tft_model_test.c; kernel choices pinned by
// bench/p30_kernel_bench.c, p30_io_bench.c, p30_fuse_probe.c.
//
// Structure (W = 16 lanes, y = x^16):
//   input   u64 limbs -> per prime residues in [0,p), one fused pass
//           (all 5 primes share the limb loads), fused REDC
//           (hi*R^2 + lo*R)*R^-1.
//   y-TFT   van der Hoeven TRUNCATED radix-2 NTT on whole 16-lane
//           vectors (vector j = limbs 16j..16j+15): 16 independent
//           scalar TFTs in lockstep, FLINT twiddle convention
//           (per-node constant w2[j] = w[2j], bit-reversed prefix-
//           stable tables as Barrett (c, c_rec) pairs). Forward output
//           truncated at tv = ceil(zlen/16) vectors; inverse consumes
//           exactly those with the zero-tail invariant.
//   pw      16-point twisted cyclic convolution mod (x^16 - w[k]) per
//           spectral vector pair (BR position k): sliding [a | a*ww]
//           windows x b-lane broadcasts, u64 accumulation, one wide
//           REDC per 8 outputs (the only Montgomery use; its uniform
//           stray R^-1 is cancelled by the final scale s = M^-1 * R).
//   output  Garner mixed-radix CRT (ascending primes, Barrett, no
//           pre-reductions) batched step-major through an L1 buffer,
//           composed to 3 limb-aligned streams, carried by two SWAR
//           chains.
//
// STAGE 2a: radix-8 moths on full nodes (3 fused levels per memory
// round-trip; the truncation spine stays radix-2), lazy [0,4p)
// between-pass ranges (Barrett eats multiplied operands raw; the
// unmultiplied operand shrinks once per level), conv16 b-lanes
// broadcast from memory. STAGE 2b: fused fwd(b)+pointwise+inverse
// DFS traversal (p30_fused_*). STAGE 2c (this revision): iterative
// full-subtree engines (p30_*_full: DFS pass order flattened to a
// leaf-block walk, moths fire on block alignment) with in-register
// m <= 32 bottoms (p30_fwd16r/inv16r/fwd32r/inv32r, fused leaves
// p30_fused8l/16l/32l split form, register kara p30_kara2r) — leaf-bench
// winners, docs §12. STAGE 2d: fully-fused truncation spine
// (p30_fused_sb, tails ride the a-spectrum's dead tail [tv, M)) +
// split fused leaves (docs §13). STAGE 2e (this revision): fused
// input column pass (p30_input_f3: decode + first 3 levels, 16-column
// row-burst tiles, NT stores; pre-depth drivers p30_*_pre enter the
// traversals 3 levels down; gated to past-L3 sizes M >= 2^17) and the
// a-canonicalize pass folded into the convs' fb loads (docs §14).
// Queued: output fusion into the last inverse pass, pq16/p30
// crossover + u64cvt dispatch leg, threading.
//
...
/* mixed fused input gate: x6 form (6 row streams = 3x2 first layer;
 * the 24-stream m3 form measured slower everywhere and is dormant).
 * Same physics as the binary gate: column-fused input only pays past
 * L3 -- at L3-resident sizes the plain input + r3fwd passes win
 * (three-way interleaved runs, docs §16c). */
#ifndef P30_MFIN_MIN_L
#define P30_MFIN_MIN_L (1 << 15)
#endif
#ifndef P30_MFIN_NT_MIN_L
#define P30_MFIN_NT_MIN_L (1 << 15)
#endif
...
static void p30_fwd_full(uint32_t *B, size_t m, size_t j,
                         const p30_prime *pp, __m512i vp, __m512i vp2);
static void p30_inv_full(uint32_t *B, size_t m, size_t j,
                         const p30_prime *pp, __m512i vp, __m512i vp2);
...
/* ------------------------------------------------------------------ */
/* truncated inverse, node (B[0..m), j), trunc z.                      */
/* Invariant (verified in tests/p30_tft_model_test.c): entry B[0..z) = */
/* spectral, B[z..m) = m*u (zeros at top); exit B[0..z) = m*u.         */
/* NOTE: no longer called by the multiply path (p30_fused_sb fuses the */
/* whole spine, tails via the a-spectrum side channel); kept as the    */
/* reference general inverse the fused traversal is derived from.      */
/* ------------------------------------------------------------------ */
...
static void p30_inv_rec(uint32_t *B, size_t m, size_t j, size_t z,
                        const p30_prime *pp, __m512i vp){
    if(m == 1 || z == 0) return;
    __m512i vp2 = _mm512_add_epi32(vp, vp);
    if(z == m && m >= 8){
        p30_inv_full(B, m, j, pp, vp, vp2);
        return;
    }
    size_t h = m >> 1;
    uint32_t *lo = B, *hi = B + P30_W * h;
    ...
    if(z < h){
        ...
        for(size_t v = z; v < h; ++v){           /* tail: h*c = (B+wB')/2 */
            __m512i a  = p30_sh2(_mm512_load_si512(lo + P30_W * v), vp2);
            __m512i wb = p30_bar2p(_mm512_load_si512(hi + P30_W * v),
                                   w.c, w.rec, vp);
            _mm512_store_si512(lo + P30_W * v,
                p30_bar2p(_mm512_add_epi32(a, wb), i2.c, i2.rec, vp));
        }
        p30_inv_rec(B, h, 2 * j, z, pp, vp);
        for(size_t v = 0; v < z; ++v){           /* m*a = 2B - w*B' */
            __m512i a  = p30_sh2(_mm512_load_si512(lo + P30_W * v), vp2);
    ...
    size_t zr = z - h;
    p30_inv_rec(B, h, 2 * j, h, pp, vp);         /* full left */
    for(size_t v = zr; v < h; ++v){              /* h*d BEFORE right rec */
        __m512i a  = p30_sh2(_mm512_load_si512(lo + P30_W * v), vp2);
        __m512i wb = p30_bar2p(_mm512_load_si512(hi + P30_W * v),
                               w.c, w.rec, vp);
        __m512i t  = _mm512_add_epi32(a, _mm512_sub_epi32(vp2, wb));
        _mm512_store_si512(hi + P30_W * v, t);
        _mm512_store_si512(lo + P30_W * v,
            _mm512_add_epi32(a, p30_sh2(t, vp2)));
    }
    p30_inv_rec(B + P30_W * h, h, 2 * j + 1, zr, pp, vp);
...
static void p30_inv_full(uint32_t *B, size_t m, size_t j,
    ...
    if(m == 8){  p30_inv8(B, 8, j, pp, vp, vp2);  return; }
    if(m == 16){ p30_inv16r(B, j, pp, vp, vp2);   return; }
    if(m == 32){ p30_inv32r(B, j, pp, vp, vp2);   return; }
    size_t LS = p30_leaf_ls(m);
    size_t nb = m / LS;
    for(size_t b = 0; b < nb; ++b){
        size_t o = b * LS;
        uint32_t *Bb = B + P30_W * o;
        size_t jb = j * nb + b;
        if(LS == 8)       p30_inv8(Bb, 8, jb, pp, vp, vp2);
        else if(LS == 16) p30_inv16r(Bb, jb, pp, vp, vp2);
        else              p30_inv32r(Bb, jb, pp, vp, vp2);
        for(size_t s = 8 * LS; s <= m; s <<= 3){     /* moths, bottom up */
...
static void p30_fused_full(uint32_t *B, const uint32_t *A, size_t m,
    ...
    if(m == 1){          /* conv16 broadcasts fb lanes: canonical copy */
        ...
        __m512i ax = _mm512_load_si512(A);
...
static void p30_fused_sb(uint32_t *B, const uint32_t *A, size_t m, size_t j,
    ...
    __m512i vp2 = _mm512_add_epi32(vp, vp);
    if(z == m){
        p30_fused_full(B, A, m, j, pp, vp, vp2);
        return;
    }
    ...
    if(z <= h){
        ...
        p30_fused_sb(lo, A, h, 2 * j, z, pp, vp);
    ...
    p30_fused_full(lo, A, h, 2 * j, pp, vp, vp2);    /* left, fused */
    ...
    p30_fused_sb(hi, A + P30_W * h, h, 2 * j + 1, zr, pp, vp);
    p30_vcc wi = p30_vcc_of(pp->winv[2 * j]);
    for(size_t v = 0; v < zr; ++v){
        __m512i a = p30_sh2(_mm512_load_si512(lo + P30_W * v), vp2);
        __m512i b = p30_sh2(_mm512_load_si512(hi + P30_W * v), vp2);
...
static void p30_fused_rec(uint32_t *B, const uint32_t *A, size_t m, size_t j,
    ...
    if(z <= h){
        ...
        p30_fused_rec(lo, A, h, 2 * j, z, pp, vp);
...
static void p30_fused_full_pre(uint32_t *B, const uint32_t *A, size_t m,
                               ...
                               __m512i vp, __m512i vp2){
    if(d == 0){ p30_fused_full(B, A, m, j, pp, vp, vp2); return; }
    ...
    p30_fused_full_pre(B, A, h, 2 * j, d - 1, pp, vp, vp2);
...
static void p30_fused_sb_pre(uint32_t *B, const uint32_t *A, size_t m,
                             ...
                             const p30_prime *pp, __m512i vp){
    if(d == 0){ p30_fused_sb(B, A, m, j, z, pp, vp); return; }
    __m512i vp2 = _mm512_add_epi32(vp, vp);
    if(z == m){ p30_fused_full_pre(B, A, m, j, d, pp, vp, vp2); return; }
    uint32_t *SB = (uint32_t *)A;            /* dead beyond local z */
    size_t h = m >> 1;
    uint32_t *lo = B, *hi = B + P30_W * h;
    p30_vcc w = p30_vcc_of(pp->w[2 * j]);
    if(z <= h){
        ...
        p30_fused_sb_pre(lo, A, h, 2 * j, z, d - 1, pp, vp);
        for(size_t v = 0; v < z; ++v){       /* m*u = 2*(h*c) - w*T   */
            __m512i a  = p30_sh2(_mm512_load_si512(lo + P30_W * v), vp2);
            __m512i u  = p30_sh2(_mm512_add_epi32(a, a), vp2);
            __m512i wb = p30_bar2p(_mm512_load_si512(SB + P30_W * (v + h)),
    ...
    p30_fused_full_pre(lo, A, h, 2 * j, d - 1, pp, vp, vp2);
    for(size_t v = zr; v < h; ++v){          /* right tails + parent out */
        __m512i c  = p30_sh2(_mm512_load_si512(lo + P30_W * v), vp2);
        __m512i wb = p30_bar2p(_mm512_load_si512(SB + P30_W * (v + h)),
                               w.c, w.rec, vp);
...
static void p30_fused_pre(uint32_t *B, const uint32_t *A, size_t m, size_t j,
    ...
    if(d == 0){ p30_fused_rec(B, A, m, j, z, pp, vp); return; }
    __m512i vp2 = _mm512_add_epi32(vp, vp);
    if(z == m){ p30_fused_full_pre(B, A, m, j, d, pp, vp, vp2); return; }
    size_t h = m >> 1;
    uint32_t *lo = B, *hi = B + P30_W * h;
    if(z <= h){                              /* fwd c-pass absorbed */
        p30_fused_pre(lo, A, h, 2 * j, z, d - 1, pp, vp);
...
static void p30_mfused(uint32_t *B, const uint32_t *A, size_t L, size_t z2v,
                       int lgL, const p30_prime *pp, __m512i vp){
    __m512i vp2 = _mm512_add_epi32(vp, vp);
    const p30_cc *tw2 = pp->rtw2[lgL];
    const p30_cc *itw1 = pp->ritw1[lgL], *itw2 = pp->ritw2[lgL];
    p30_r3fwd(B, L, lgL, pp, vp, vp2);
    p30_fused_full(B,             A,             L, 0, pp, vp, vp2);
    p30_fused_full(B + P30_W * L, A + P30_W * L, L, 0, pp, vp, vp2);
    ...
    p30_fused_sb(hi, A + P30_W * 2 * L, L, 0, z2v, pp, vp);
...
/* mixed drivers with radix-3 + twist + first 3 binary levels
 * pre-applied by the fused input pass (p30_input_m3): no r3fwd, and
 * the children enter the existing pre-depth machinery at d = 3. */
static void p30_mfwd_pre(uint32_t *B, size_t L, size_t z2v,
                         const p30_prime *pp, __m512i vp){
    p30_fwd_pre(B,                 L, 0, L,   3, pp, vp);
    p30_fwd_pre(B + P30_W * L,     L, 0, L,   3, pp, vp);
    p30_fwd_pre(B + P30_W * 2 * L, L, 0, z2v, 3, pp, vp);
}

static void p30_mfused_pre(uint32_t *B, const uint32_t *A, size_t L,
                           size_t z2v, int lgL, const p30_prime *pp,
                           __m512i vp){
    __m512i vp2 = _mm512_add_epi32(vp, vp);
    const p30_cc *tw2 = pp->rtw2[lgL];
    const p30_cc *itw1 = pp->ritw1[lgL], *itw2 = pp->ritw2[lgL];
    p30_fused_full_pre(B,             A,             L, 0, 3, pp, vp, vp2);
    p30_fused_full_pre(B + P30_W * L, A + P30_W * L, L, 0, 3, pp, vp, vp2);
    uint32_t *lo = B, *mid = B + P30_W * L, *hi = B + P30_W * 2 * L;
    p30_vcc zz1 = p30_vcc_of(pp->zt), zz2 = p30_vcc_of(pp->zt2);
    p30_vcc i1z = p30_vcc_of(pp->i1z), c3 = p30_vcc_of(pp->c3);
    __m512i lc2  = _mm512_loadu_si512(pp->lv2[lgL].c);
    ...
    p30_fused_sb_pre(hi, A + P30_W * 2 * L, L, 0, z2v, 3, pp, vp);
    for(size_t v = 0; v < z2v; ++v){
        p30_vcc it1 = p30_vcc_of(itw1[v]);
        p30_vcc it2 = p30_vcc_of(itw2[v]);
        __m512i c0 = p30_sh2(_mm512_load_si512(lo + P30_W * v), vp2);
        __m512i c1 = p30_bar2pv(
...
/* ------------------------------------------------------------------ */
/* x6 mixed fused input (user-shaped): absorb radix-3 x 2 = the 3x2    */
/* first-layer decomposition only -- SIX output row streams (the 24-   */
/* stream m3 thrashed; binary f3's 8 streams are the proven regime).   */
/* Matrix: NV = 3L vectors as 6 rows (child t, half r1) x C = L/2      */
/* columns. The absorbed binary level is the cyclic child top: its     */
/* twiddle is w[0] = 1 -- multiply-free butterflies -- and its INVERSE */
/* combine fuses into the radix-3 seam pass (p30_mfused_x6), so the    */
/* inverse side costs no extra pass. Children enter at d = 1 via the   */
/* existing pre-depth machinery.                                       */
/* ------------------------------------------------------------------ */
static void p30_input_x6(uint32_t *out[P30_NP], const uint64_t *a, size_t n,
                         size_t L, size_t z2v, int lgL, const p30_pset *ps,
                         int nt){
    const __m512i PK = p30_pack_idx();
    size_t C = L >> 1;
    int hi2 = z2v > C;                       /* child-2 hi half demanded */
    size_t nf = n / P30_W;
    size_t rem = n % P30_W;
    __mmask8 m0 = (__mmask8)((rem >= 8) ? 0xFF : ((1u << rem) - 1u));
    __mmask8 m1 = (__mmask8)((rem >= 8) ? ((1u << (rem - 8)) - 1u) : 0);
    __m512i tb[6][16];                       /* 6KB L1 staging tile */
    for(size_t ct = 0; ct < C; ct += 16){
        ...
        for(int q = 0; q < P30_NP; ++q){
            ...
            __m512i vp2 = _mm512_add_epi32(vp, vp);
            /* decode the 6 row-bursts (row (t3, r1) base t3*L + r1*C) */
            for(size_t rr = 0; rr < 6; ++rr){
                size_t v0 = (rr / 2) * L + (rr & 1) * C + ct;
                for(size_t t = 0; t < tw; ++t){
                    size_t v = v0 + t;
                    if(v > nf || (v == nf && !rem)){
                        tb[rr][t] = _mm512_setzero_si512();
                        continue;
                    }
                    __m512i x0, x1;
                    if(v < nf){
                    ...
                    __m512i rv = _mm512_permutex2var_epi32(r0, PK, r1);
                    rv = p30_shrink(rv, vp2);
                    tb[rr][t] = p30_shrink(rv, vp);
                }
            }
            /* radix-3 + twist per half-row, then the binary top level */
            {   p30_vcc z1c = p30_vcc_of(pp->zt), z2c = p30_vcc_of(pp->zt2);
                const p30_cc *tw1t = pp->rtw1[lgL], *tw2t = pp->rtw2[lgL];
                __m512i lc1 = _mm512_loadu_si512(pp->lv1[lgL].c);
                __m512i lr1 = _mm512_loadu_si512(pp->lv1[lgL].rec);
                __m512i lc2 = _mm512_loadu_si512(pp->lv2[lgL].c);
                __m512i lr2 = _mm512_loadu_si512(pp->lv2[lgL].rec);
                for(size_t t = 0; t < tw; ++t){
                    ...
                    for(size_t r1 = 0; r1 < 2; ++r1){
                        size_t v = r1 * C + ct + t;     /* child-local */
                        __m512i aa = tb[r1][t], bb = tb[2 + r1][t],
                                dd = tb[4 + r1][t];
                        __m512i t0 = _mm512_add_epi32(
                            _mm512_add_epi32(aa, bb), dd);
                        __m512i y1 = _mm512_add_epi32(
                            p30_sh2(_mm512_add_epi32(aa,
                                p30_bar2p(bb, z1c.c, z1c.rec, vp)), vp2),
                            p30_bar2p(dd, z2c.c, z2c.rec, vp));
                        __m512i y2 = _mm512_add_epi32(
                            p30_sh2(_mm512_add_epi32(aa,
                                p30_bar2p(bb, z2c.c, z2c.rec, vp)), vp2),
                            p30_bar2p(dd, z1c.c, z1c.rec, vp));
                        p30_vcc w1 = p30_vcc_of(tw1t[v]);
                        p30_vcc w2 = p30_vcc_of(tw2t[v]);
                        x[r1]     = t0;                 /* < 3p */
                        x[2 + r1] = p30_bar2pv(
                            p30_bar2p(y1, w1.c, w1.rec, vp), lc1, lr1, vp);
                        x[4 + r1] = p30_bar2pv(
                            p30_bar2p(y2, w2.c, w2.rec, vp), lc2, lr2, vp);
                    }
                    /* binary top level per child, twiddle-free (w[0]=1) */
                    for(size_t t3 = 0; t3 < 3; ++t3){
                        __m512i aa = p30_sh2(x[2*t3], vp2);
                        __m512i bb = p30_sh2(x[2*t3 + 1], vp2);
                        tb[2*t3][t] = _mm512_add_epi32(aa, bb);
                        if(t3 < 2 || hi2)
                            tb[2*t3 + 1][t] = _mm512_add_epi32(aa,
                                _mm512_sub_epi32(vp2, bb));
                    }
                }
            }
            /* row-burst stores (5 or 6 streams) */
            for(size_t rr = 0; rr < 6; ++rr){
                if(rr == 5 && !hi2) continue;
                uint32_t *dst = out[q]
                    + P30_W * ((rr / 2) * L + (rr & 1) * C + ct);
                if(nt)
                    for(size_t t = 0; t < tw; ++t)
                    ...
                    for(size_t t = 0; t < tw; ++t)
                        _mm512_store_si512(dst + P30_W * t, tb[rr][t]);
            }
        }
    }
    if(nt) _mm_sfence();
...
static void p30_mfwd_x6(uint32_t *B, size_t L, size_t z2v,
                        const p30_prime *pp, __m512i vp){
    p30_fwd_pre(B,                 L, 0, L,   1, pp, vp);
    p30_fwd_pre(B + P30_W * L,     L, 0, L,   1, pp, vp);
    p30_fwd_pre(B + P30_W * 2 * L, L, 0, z2v, 1, pp, vp);
}

/* b-side: halves of children 0,1 fused directly; ONE final pass fuses
 * their twiddle-free d=1 inverse combines with the radix-3 seam (no
 * extra pass vs the unfused path); child 2 via fused_sb_pre(d=1);
 * 3-point combine as before. */
static void p30_mfused_x6(uint32_t *B, const uint32_t *A, size_t L,
                          size_t z2v, int lgL, const p30_prime *pp,
                          __m512i vp){
    __m512i vp2 = _mm512_add_epi32(vp, vp);
    size_t C = L >> 1;
    const p30_cc *tw2 = pp->rtw2[lgL];
    const p30_cc *itw1 = pp->ritw1[lgL], *itw2 = pp->ritw2[lgL];
    for(size_t t3 = 0; t3 < 2; ++t3){        /* children 0,1 halves */
        uint32_t *Bc = B + P30_W * t3 * L;
        const uint32_t *Ac = A + P30_W * t3 * L;
        p30_fused_full(Bc,             Ac,             C, 0, pp, vp, vp2);
        p30_fused_full(Bc + P30_W * C, Ac + P30_W * C, C, 1, pp, vp, vp2);
    }
    uint32_t *lo = B, *mid = B + P30_W * L, *hi = B + P30_W * 2 * L;
    p30_vcc zz1 = p30_vcc_of(pp->zt), zz2 = p30_vcc_of(pp->zt2);
    p30_vcc i1z = p30_vcc_of(pp->i1z), c3 = p30_vcc_of(pp->c3);
    ...
    __m512i ilr2 = _mm512_loadu_si512(pp->ilv2[lgL].rec);
    uint32_t *SB2 = (uint32_t *)(A + P30_W * 2 * L);
    /* fused pass: per v, d=1 combines (children 0,1) feed either the
     * in-place ct store (3-pt region) or the seam (positions >= z2) */
#define P30_X6C(child_, v_, out0_, out1_) do{                                    uint32_t *bc_ = (child_) ? mid : lo;                                     __m512i a_ = p30_sh2(_mm512_load_si512(bc_ + P30_W * (v_)), vp2);        __m512i b_ = p30_sh2(_mm512_load_si512(bc_ + P30_W * ((v_) + C)),                             vp2);                                               out0_ = _mm512_add_epi32(a_, b_);                                        out1_ = _mm512_add_epi32(a_, _mm512_sub_epi32(vp2, b_));             }while(0)
#define P30_X6SEAM(p_, c0_, c1_) do{                                             p30_vcc it1_ = p30_vcc_of(itw1[p_]);                                     p30_vcc rw2_ = p30_vcc_of(tw2[p_]);                                      __m512i cc0_ = p30_sh2(c0_, vp2);                                        __m512i cc1_ = p30_bar2pv(p30_bar2p(c1_, it1_.c, it1_.rec, vp),                                    ilc1, ilr1, vp);                               __m512i u1_ = p30_bar2p(                                                     _mm512_add_epi32(cc0_, _mm512_sub_epi32(vp2, cc1_)),                     i1z.c, i1z.rec, vp);                                                 __m512i u0_ = p30_sh2(                                                       _mm512_add_epi32(cc0_, _mm512_sub_epi32(vp2, u1_)), vp2);            __m512i tt_ = p30_sh2(                                                       _mm512_add_epi32(u0_, p30_bar2p(u1_, zz2.c, zz2.rec, vp)),               vp2);                                                                _mm512_store_si512(SB2 + P30_W * (p_),                                       p30_bar2pv(p30_bar2p(tt_, rw2_.c, rw2_.rec, vp),                                    lc2, lr2, vp));                                           _mm512_store_si512(lo + P30_W * (p_),                                                       p30_bar2p(u0_, c3.c, c3.rec, vp));                    _mm512_store_si512(mid + P30_W * (p_),                                                      p30_bar2p(u1_, c3.c, c3.rec, vp));                }while(0)
    {
        size_t b = z2v > C ? z2v - C : 0;    /* below b: v+C is 3-pt   */
        size_t e = z2v < C ? z2v : C;        /* below e: v   is 3-pt   */
        for(size_t v = 0; v < C; ++v){
            __m512i c0a, c0b, c1a, c1b;
            P30_X6C(0, v, c0a, c0b);
            P30_X6C(1, v, c1a, c1b);
            if(v < e){                       /* pos v: store for 3-pt */
                _mm512_store_si512(lo + P30_W * v, c0a);
                _mm512_store_si512(mid + P30_W * v, c1a);
            }else{
                P30_X6SEAM(v, c0a, c1a);
            }
            if(v < b){                       /* pos v+C: store        */
                _mm512_store_si512(lo + P30_W * (v + C), c0b);
                _mm512_store_si512(mid + P30_W * (v + C), c1b);
            }else{
                P30_X6SEAM(v + C, c0b, c1b);
            }
        }
    }
#undef P30_X6C
#undef P30_X6SEAM
    p30_fused_sb_pre(hi, A + P30_W * 2 * L, L, 0, z2v, 1, pp, vp);
    for(size_t v = 0; v < z2v; ++v){         /* 3-pt combine, unscaled */
        p30_vcc it1 = p30_vcc_of(itw1[v]);
        p30_vcc it2 = p30_vcc_of(itw2[v]);
        __m512i c0 = p30_sh2(_mm512_load_si512(lo + P30_W * v), vp2);
...
/* ------------------------------------------------------------------ */
/* mixed fused input: decode + radix-3 + twist + the first 3 binary    */
/* levels of EACH child, one tiled pass (24 rows = 3 children x 8;     */
/* C = L/8 columns). Children 0,1 full; child 2 stores rows < zrow2 =  */
/* ceil(z2 / C). Replaces p30_input + the whole p30_r3fwd pass; the    */
/* traversals enter via the d = 3 pre-depth drivers per child.         */
/* ------------------------------------------------------------------ */
static void p30_input_m3(uint32_t *out[P30_NP], const uint64_t *a, size_t n,
                         size_t L, size_t z2v, int lgL, const p30_pset *ps,
                         int nt){
    const __m512i PK = p30_pack_idx();
    size_t C = L >> 3;
    size_t zrow2 = (z2v + C - 1) / C;
    size_t nf = n / P30_W;
    size_t rem = n % P30_W;
    __mmask8 m0 = (__mmask8)((rem >= 8) ? 0xFF : ((1u << rem) - 1u));
    __mmask8 m1 = (__mmask8)((rem >= 8) ? ((1u << (rem - 8)) - 1u) : 0);
    __m512i tb[24][16];                      /* 24KB L1 staging tile */
    for(size_t ct = 0; ct < C; ct += 16){
        size_t tw = (ct + 16 < C ? 16 : C - ct);
        for(int q = 0; q < P30_NP; ++q){
            const p30_prime *pp = &ps->pr[q];
            ...
            __m512i vp  = _mm512_set1_epi32((int)pp->p);
            __m512i vp2 = _mm512_add_epi32(vp, vp);
            /* decode the 24 row-bursts */
            for(size_t r3 = 0; r3 < 3; ++r3)
                for(size_t r = 0; r < 8; ++r){
                    size_t v0 = r3 * L + r * C + ct;
                    for(size_t t = 0; t < tw; ++t){
                        size_t v = v0 + t;
                        if(v > nf || (v == nf && !rem)){
                            tb[8*r3 + r][t] = _mm512_setzero_si512();
                            continue;
                        }
                        __m512i x0, x1;
                        if(v < nf){
                        ...
                        __m512i rr = _mm512_permutex2var_epi32(r0, PK, r1);
                        rr = p30_shrink(rr, vp2);
                        tb[8*r3 + r][t] = p30_shrink(rr, vp);
                    }
                }
            /* radix-3 + twist (per sub-row r, twist index v = r*C+ct+t) */
            {   p30_vcc z1c = p30_vcc_of(pp->zt), z2c = p30_vcc_of(pp->zt2);
                const p30_cc *tw1t = pp->rtw1[lgL], *tw2t = pp->rtw2[lgL];
                __m512i lc1 = _mm512_loadu_si512(pp->lv1[lgL].c);
                __m512i lr1 = _mm512_loadu_si512(pp->lv1[lgL].rec);
                ...
                for(size_t r = 0; r < 8; ++r)
                    for(size_t t = 0; t < tw; ++t){
                        ...
                        tb[8 + r][t] = p30_bar2pv(
                            p30_bar2p(y1, w1.c, w1.rec, vp), lc1, lr1, vp);
                        tb[16 + r][t] = p30_bar2pv(
                            p30_bar2p(y2, w2.c, w2.rec, vp), lc2, lr2, vp);
                    }
            }
            /* per-child 3-level binary networks (in-tile) */
            for(size_t r3 = 0; r3 < 3; ++r3){
                size_t zr = r3 == 2 ? zrow2 : 8;
                for(size_t t = 0; t < tw; ++t){
                    __m512i x[8];
                    for(size_t r = 0; r < 8; ++r) x[r] = tb[8*r3 + r][t];
                    p30_cfwd8x(x, zr, pp, vp, vp2);
                    for(size_t r = 0; r < zr; ++r) tb[8*r3 + r][t] = x[r];
                }
            }
            /* row-burst stores */
            for(size_t r3 = 0; r3 < 3; ++r3){
                size_t zr = r3 == 2 ? zrow2 : 8;
                for(size_t r = 0; r < zr; ++r){
                    uint32_t *dst = out[q] + P30_W * (r3 * L + r * C + ct);
                    if(nt)
                        for(size_t t = 0; t < tw; ++t)
                            _mm512_stream_si512((void *)(dst + P30_W * t),
                                                tb[8*r3 + r][t]);
                        ...
                        for(size_t t = 0; t < tw; ++t)
                            _mm512_store_si512(dst + P30_W * t,
                                               tb[8*r3 + r][t]);
                }
            }
        }
...
static inline int p30_mul_r(uint64_t *rp,
    ...
    int mfin = mixed && (L >= P30_MFIN_MIN_L);
    int mnt  = mfin && (L >= P30_MFIN_NT_MIN_L);
    if(fin){
        size_t zrow = (tv + (M >> 3) - 1) / (M >> 3);
        p30_input_f3(ra, ap, (size_t)an, M, zrow, ps, nt);
        p30_input_f3(rb, bp, (size_t)bn, M, zrow, ps, nt);
    }else if(mfin){
        size_t z2v = (size_t)tv - 2 * L;
        p30_input_x6(ra, ap, (size_t)an, L, z2v, lg - 2, ps, mnt);
        p30_input_x6(rb, bp, (size_t)bn, L, z2v, lg - 2, ps, mnt);
    }else{
    ...
    for(int q = 0; q < P30_NP; ++q){
        ...
        if(mixed && mfin){
            p30_mfwd_x6(ra[q], L, (size_t)tv - 2 * L, pp, vp);
            p30_mfused_x6(rb[q], ra[q], L, (size_t)tv - 2 * L, lg - 2,
                          pp, vp);
        }else if(mixed){
            p30_mfwd(ra[q], L, (size_t)tv - 2 * L, lg - 2, pp, vp);
            p30_mfused(rb[q], ra[q], L, (size_t)tv - 2 * L, lg - 2, pp, vp);
        }else if(fin){
            p30_fwd_pre(ra[q], M, 0, tv, 3, pp, vp);
            p30_fused_pre(rb[q], ra[q], M, 0, tv, 3, pp, vp);
        }else{
            p30_fwd_rec(ra[q], M, 0, tv, pp, vp);
            p30_fused_rec(rb[q], ra[q], M, 0, tv, pp, vp);
        }
